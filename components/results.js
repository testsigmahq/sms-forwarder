...
const Result = () => {
  ...
  const [isBackgroundServiceRunning, setIsBackgroundServiceRunning] = useState(
  ...
  const signInSilently = async (retries = 3) => {
    try {
      ...
      console.log(
        getCurrentTime("INFO") + "Silent sign-in successful, user::",
        user
      );
      return user;
    } catch (error) {
      console.error(
        getCurrentTime("ERROR") + "Silent sign-in failed, Error::",
        error
      );
      if (retries > 0) {
  ...
  const signInInteractively = async () => {
    try {
      ...
      console.log(
        getCurrentTime("INFO") +
          "Silent sign-in signInInteractively successful user::",
        user
      );
      return user;
    } catch (error) {
      console.error(
        getCurrentTime("ERROR") + "Interactive sign-in failed, Error::",
        error
      );
    }
  };
  ...
  const startBackgroundTask = async () => {
    try {
      ...
      console.error(
        getCurrentTime("ERROR") + "Error starting background task, Error::",
        error
      );
    }
  };
  ...
  const updateBackgroundNotification = async () => {
    try {
      if (BackgroundService.isRunning()) {
        ...
        console.log(
          getCurrentTime("INFO") + "Background notification updated."
        );
      }
    } catch (error) {
      console.error(
        getCurrentTime("ERROR") +
          "Error updating background notification, Error::",
        error
      );
    }
  };
  ...
  const stopBackgroundTask = async () => {
    try {
      ...
      console.error(
        getCurrentTime("ERROR") + "Error stopping background task, Error::",
        error
      );
    }
  };
  ...
  const fetchLatestMessage = () => {
    ...
    SmsAndroid.list(
      ...
      (fail) => {
        console.log(
          getCurrentTime("ERROR") + "Failed to fetch SMS list, Error::",
          fail
        );
      },
      async (count, smsList) => {
        ...
        if (arr.length > 0) {
          ...
          if (latestObject._id > (lastMessage?.date || 0)) {
            ...
            fetchResult().then(() =>
              console.log(getCurrentTime("INFO") + "Fetched result")
            );
          }
        }
      }
  ...
  const fetchResult = async () => {
    Database.readResults()
      ...
      .catch((error) => {
        console.log(
          getCurrentTime("ERROR") +
            "Error occurred while fetching data, Error::",
          error
        );
      });
  };
  ...
  const forwardMessage = (latestObject) => {
    ...
    fetchFilters("active", latestObject).then(() =>
      console.log(getCurrentTime("INFO") + "success")
    );
  };
  ...
  async function fetchFilters(status, latestObject) {
    try {
      ...
      console.error(
        getCurrentTime("ERROR") +
          "Error occurred while fetching filters, Error::",
        error
      );
    }
  }
  ...
  const getMailMethod = async () => {
    try {
      ...
      if (authSettings) {
        ...
        console.log(
          getCurrentTime("INFO") + "AuthSettings not found or error occurred."
        );
      ...
      console.error(
        getCurrentTime("ERROR") +
          "Error occurred while fetching auth settings, Error::",
        error
      );
      return undefined;
    }
  ...
  const getAccessToken = async () => {
    try {
      ...
      const response = await axios.post("https://oauth2.googleapis.com/token", {
        ...
        client_id:
          "473722209735-7tcidkd4hji670ckn20g9r6eu2dlivit.apps.googleusercontent.com",
      ...
      if (error.response) {
        console.error(
          getCurrentTime("ERROR") + "Error retrieving access token:",
          error.response.status,
          error.response.data
        );
      } else if (error.request) {
        console.error(
          getCurrentTime("ERROR") + "No response received:",
          error.request
        );
      } else {
        console.error(
          getCurrentTime("ERROR") + "Error setting up request:",
          error.message
        );
      }
      ...
      Database.fetchUserById(1)
        .then((e) => {
          if (e) {
            ...
            console.log(
              getCurrentTime("DEBUG") + "User data not found for ID 1."
            );
          }
        })
        .catch((error) => {
  ...
  async function fetchRecipients(id, latestObject) {
    try {
      ...
      console.log(
        getCurrentTime("INFO") + "New message return :: ",
        newMessage
      );

      if (recipients?.phoneNumbers) {
        const phoneNumbersRecipient = recipients?.phoneNumbers?.map(
          (item) => item.text
        );
        if (messageCondition) {
          try {
            ...
            for (const phoneNumber of phoneNumbersRecipient) {
              await Database.insertResults(
                newMessage,
                latestObject?.address,
                phoneNumber,
            ...
            console.error(
              getCurrentTime("ERROR") + "Error occurred while sending SMS:",
              error
            );
          }
        }
      ...
      if (recipients?.urls) {
        if (messageCondition) {
          recipients?.urls?.forEach((url) => {
            const isForwarded = forwardToURL(
              url.requestMethod,
              url.text,
              url.key,
              latestObject?.body
            ...
            if (isForwarded) {
              ...
              console.error(
                getCurrentTime("ERROR") +
                  "Error occurred while forwarding URL:",
                error
              );
            }
          });
        ...
        if (messageCondition) {
          ...
          for (const email of recipients.emails) {
            if (mailMethod === "Via Gmail API") {
              const isForwarded = await sendEmail(
                email.text,
                latestObject?.address,
                newMessage
              );
              ...
              if (smtpRef.current) {
                const isForwarded = sendEmailSmtp(
                  email.text,
                  latestObject?.address,
                  newMessage
                );
      ...
      console.error(
        getCurrentTime("ERROR") +
          "Error occurred while fetching recipients, Error::",
        error
      );
    }
  }
  ...
  async function canForwardMessage(id, latestObject) {
    ...
    console.log(
      getCurrentTime("DEBUG") + "forwardCondition::",
      forwardCondition
    );
    if (rule.senderNumbers.length > 0 || rule.texts.length > 0) {
      if (rule.senderNumbers.length > 0) {
        ...
        rule.senderNumbers.forEach((number) => {
          console.log(
            ...
            number.sender,
            "\n\n slicing : \t,",
            latestObject.address.slice(3),
            "\n\n status \t",
            number.sendStatus
          );
          if (
            number.sender === latestObject.address.slice(3) &&
            number.sendStatus === "1"
          ) {
      ...
      if (rule.texts.length > 0) {
        ...
        for (const text of rule.texts) {
          if (
            text.sendStatus === "1" &&
            !latestObject?.body.includes(text.messageText)
          ) {
            booleanValue = false;
            console.log(
              getCurrentTime("DEBUG") +
                "Send status is 1 and message not found ::",
              text.messageText,
              booleanValue
            );
            return;
          ...
          if (
            ...
            booleanValue = false;
            console.log(
              getCurrentTime("DEBUG") + "Send status is 0 and message found ::",
              text.messageText,
              booleanValue
            );
            return;
          ...
          booleanValue = true;
          console.log(
            getCurrentTime("DEBUG") + "Send status is 1 and message found ::",
            text.messageText,
            booleanValue
          );
        }
      ...
      console.log(
        getCurrentTime("DEBUG") +
          "No text and sender rule found. Returning true"
      );
      booleanValue = true;
    }
    console.log(
      getCurrentTime("DEBUG") + "canForwardMessage ::\t",
      booleanValue
    );
    return booleanValue;
  }
    ...
    try {
      ...
      if (method === "post") {
        axios
          ...
          .then((response) => {
            console.log(
              getCurrentTime("INFO") + "SMS forwarded successfully:",
              response.data
            );
          })
          .catch((error) => {
            ...
            console.error(
              getCurrentTime("ERROR") + "Failed to forward SMS, Error::",
              error
            );
          });
      } else if (method === "get") {
        axios
          ...
          .catch((error) => {
            ...
            console.error(
              getCurrentTime("ERROR") + "Failed to forward SMS:",
              error
            );
          });
      }
  ...
  const requestSMSPermissions = async (retryCount = 0) => {
    ...
    try {
      ...
      if (readSmsGranted && sendSmsGranted) {
        ...
        console.log(
          getCurrentTime("INFO") + "SMS permissions denied. Retrying..."
        );
        await requestSMSPermissions(retryCount + 1);
      } else {
        console.log(
          getCurrentTime("INFO") +
            "SMS permissions denied after multiple attempts."
        );
      }
    } catch (err) {
      showMessage({
      ...
      console.error(
        getCurrentTime("ERROR") + "Error while requesting permissions:",
        err
      );
    }
  };
  ...
  async function deleteById(id) {
    try {
      ...
      console.error(
        getCurrentTime("ERROR") + "Error deleting result, Error:",
        error
      );
    }
  }
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.header}>
        <Text style={styles.title}>Forwarded Logs</Text>
      </View>

      <View style={[styles.blackCard]}>
        <Text style={{ fontSize: 15, top: 5 }}>
          Enable SMS Listening and Forwarding
        </Text>
        <TouchableOpacity
          style={styles.optionContainer}
          onPress={handleBackGroundService}
        >
          <View
            style={[
              styles.toggleButton,
              isBackgroundServiceRunning && styles.toggleButtonActive,
            ]}
          >
            <View
              style={[
                styles.toggleKnob,
                isBackgroundServiceRunning && styles.toggleKnobActive,
              ]}
            />
          </View>
        </TouchableOpacity>
          ...
          <View style={styles.modalContent}>
            <View style={{ padding: 10 }}>
              <Text style={{ fontWeight: "bold", fontSize: 15 }}>
                From : {modelResult?.sender}
              </Text>
              <Text style={{ fontWeight: "bold", fontSize: 15 }}>
                To : {modelResult?.receiver}
              </Text>
